...
def get_predicted_output(pars, df):
    pred_output = []

    cert_su = [calc_subjective_utility(i, pars[0], pars[1]) for i in df.cert]
    loss_su = [calc_subjective_utility(-1 * i, pars[0], pars[1]) for i in df.loss]
    gain_su = [calc_subjective_utility(i, pars[0], pars[1]) for i in df.gain]
    gamble_cert_diff = calc_utility_diff(gain_su, loss_su, cert_su)
    prob_accept = calc_prob_accept(gamble_cert_diff, mu = pars[2])
    n_pred_accepted = np.sum(prob_accept > 0.5)
    n_accepted = np.sum(df.response)
    pred_acc = np.mean((prob_accept > 0.5) == df.response)
    pred_output = {"n_pred_accepted": n_pred_accepted, 
